...
impl WslBridge {
    ...
    pub async fn detect_wsl_status(target_distro: &str) -> Result<WslStatusInfo> {
        #[cfg(target_os = "windows")]
        {
            ...
            let target_found = distros.iter().any(|d| d.name.eq_ignore_ascii_case(target_distro));
            let target_running = distros.iter().any(|d| {
                d.name.eq_ignore_ascii_case(target_distro) && d.state.eq_ignore_ascii_case("Running")
            });

            // Check default version
            let is_v2 = distros.iter().any(|d| d.name.eq_ignore_ascii_case(target_distro) && d.version == 2) 
...
"#;
                ...
                if let Some(last_line) = stdout.lines().last() {
                    if let Ok(val) = serde_json::from_str::<serde_json::Value>(last_line.trim()) {
                        ...
                        return Ok(RocmStatusInfo {
                            ...
                            gpu_device_name: gpu_name.or_else(|| Some("AMD Radeon RX 7700 XT".to_string())),
    ...
    pub fn parse_wsl_list_output(text: &str) -> Vec<WslDistroInfo> {
        ...
        for line in text.lines() {
            ...
            if line_trimmed.is_empty() || line_trimmed.starts_with("NAME") || line_trimmed.starts_with("---") {
                continue;
            }
