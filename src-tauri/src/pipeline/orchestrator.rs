...
impl PipelineOrchestrator {
    ...
    pub async fn set_input_video(&self, win_video_path: &str, distro: &str) {
        ...
        st.input_video_path_wsl = PathMapper::win_to_wsl(win_video_path);
        
        let out_wsl = format!("{}_dubbed_zh.mp4", st.input_video_path_wsl.trim_end_matches(".mp4"));
        st.output_video_path_wsl = Some(out_wsl.clone());
        st.output_video_path_win = Some(PathMapper::wsl_to_win(&out_wsl, distro));

        let meta_wsl = format!("{}_utterance_metadata.json", st.input_video_path_wsl.trim_end_matches(".mp4"));
        st.metadata_json_path_wsl = Some(meta_wsl.clone());
        st.metadata_json_path_win = Some(PathMapper::wsl_to_win(&meta_wsl, distro));
    ...
    pub async fn run_single_stage(
        ...
        if !res.success {
            if stage_id == PipelineStageId::Lipsync && config.lipsync_fallback_on_oom && res.error_kind == Some(ProcessErrorKind::OutOfMemoryGpu) {
                if let Some(ref tx) = log_tx {
                    let _ = tx.send(ProcessLogLine {
                        stream: "system".to_string(),
                ...
                let fallback_cmd = self.build_stage_command(stage_id, &input_wsl, &output_wsl, &meta_wsl, &fallback_config);
                ...
                if retry_res.success {
                    ...
                    s.progress_percent = 100.0;
                    s.completed_at_ms = Some(chrono::Utc::now().timestamp_millis());
                    s.user_suggestion = Some("Úspešne dokončené cez záchranný MuseTalk engine.".to_string());
                    return Ok(());
                }
            }
            ...
            return Err(anyhow::anyhow!("Fáza {} zlyhala: {}", s.name, res.stderr));
        }
        ...
        for step in 1..=5 {
            ...
            if let Some(ref tx) = log_tx {
                let _ = tx.send(ProcessLogLine {
                    ...
                    message: format!("[SIMULÁCIA] {}: Spracúvam krok {}/5 ({} %)...", stage_name, step, pct),
                    timestamp_ms: chrono::Utc::now().timestamp_millis(),
                    is_progress: true,
