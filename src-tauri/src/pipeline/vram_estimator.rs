    ...
    pub fn calculate_budget(config: &AppConfig) -> FullPipelineResourceBudget {
        ...
        let (asr_vram, asr_ram, asr_gpu, asr_warn) = match (config.asr_engine, config.asr_device) {
            (_, AsrDevice::Cpu) => (
                ...
                3500,
                false,
                Some("ASR beží na CPU — spracovanie bude pomalšie, ale ušetrí VRAM.".to_string()),
            ),
            (crate::config::app_config::AsrEngine::WhisperSk, AsrDevice::GpuRocm) => {
                (5600, 4200, true, None)
            }
            (crate::config::app_config::AsrEngine::FasterWhisper, AsrDevice::GpuRocm) => (
                4200,
                3200,
                true,
                Some("faster-whisper vyžaduje overenie ROCm podpory v CTranslate2.".to_string()),
            ),
        };
        stages.push(StageResourceEstimate {
        ...
        let (tts_vram, tts_ram, tts_gpu, tts_warn, tts_rec) = match config.tts_engine {
            TtsEngine::Piper => (
                ...
                false,
                None,
                Some("Piper TTS je komerčne bezpečný (MIT) a ultra-ľahký na RAM/VRAM.".to_string()),
            ),
            TtsEngine::Kokoro => (
                1800,
                1500,
                true,
                None,
                Some("Kokoro TTS poskytuje vysokú kvalitu s Apache 2.0 licenciou.".to_string()),
            ),
            TtsEngine::CoquiXtts => (
                3600,
                3200,
                true,
                Some("UPOZORNENIE: Coqui XTTS-v2 je pod CPML nekomerčnou licenciou.".to_string()),
                None,
            ),
        };
        stages.push(StageResourceEstimate {
