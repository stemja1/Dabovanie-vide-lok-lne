...
impl UtteranceMetadataDocument {
    ...
    pub fn recalculate_timings(&mut self) {
        self.utterances.sort_by(|a, b| a.start_time.partial_cmp(&b.start_time).unwrap_or(std::cmp::Ordering::Equal));
    ...
    pub fn validate_integrity(&self) -> Result<()> {
        for (idx, utt) in self.utterances.iter().enumerate() {
            ...
            if utt.start_time < 0.0 {
                bail!("Utterance {} má záporný začiatočný čas: {}", utt.id, utt.start_time);
            }
            if utt.end_time <= utt.start_time {
                bail!("Utterance {} má neplatné časovanie: start={}s, end={}s", utt.id, utt.start_time, utt.end_time);
            }
            if utt.speed_factor <= 0.0 || utt.speed_factor > 4.0 {
                bail!("Utterance {} má neplatný speed factor: {}", utt.id, utt.speed_factor);
            }
        }
        Ok(())
    }
    ...
    pub fn split_utterance(
        ...
        let pos = self.utterances.iter().position(|u| u.id == utterance_id)
        ...
        if split_time <= orig.start_time || split_time >= orig.end_time {
            bail!("Čas rozdelenia ({:.2}s) musí byť v intervale ({:.2}s, {:.2}s)", split_time, orig.start_time, orig.end_time);
        }
    ...
    pub fn merge_utterances(&mut self, id1: &str, id2: &str) -> Result<()> {
        let pos1 = self.utterances.iter().position(|u| u.id == id1)
            .ok_or_else(|| anyhow::anyhow!("Utterance 1 '{}' nebol nájdený", id1))?;
        let pos2 = self.utterances.iter().position(|u| u.id == id2)
            .ok_or_else(|| anyhow::anyhow!("Utterance 2 '{}' nebol nájdený", id2))?;

        if (pos1 as isize - pos2 as isize).abs() != 1 {
            bail!("Zlúčiť je možné iba susediace segmenty (nájdené pozície: {}, {})", pos1, pos2);
        }

        let (first_idx, second_idx) = if pos1 < pos2 { (pos1, pos2) } else { (pos2, pos1) };
